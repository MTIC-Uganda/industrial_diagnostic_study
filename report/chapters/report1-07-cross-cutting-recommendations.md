# Chapter 7: Cross-Cutting Priority Actions and Policy Recommendations

**Report:** Report 1 — Iron & Steel, Copper & Allied Metals, and Automotive Value Chains
**Status:** Draft for validation

---

## 7.1 Introduction

Iron & Steel, Copper & Allied Metals, and Automotives share a common set of binding constraints and a common industrial logic. This chapter consolidates the cross-cutting actions that would lift all three chains simultaneously, and maps them to the six priority areas of the Uganda–UNIDO Programme for Country Partnership (PCP). These recommendations complement — and should be read alongside — the chain-specific action matrices in Chapters 4–6.

The integrating insight of Report 1 is that **Iron & Steel, Copper, and Automotive form a single industrial system**: mined and processed metal feeds fabricated products that feed vehicle manufacture. Constraints that raise the cost of metal — energy, logistics, finance, feedstock policy — cascade through all three. Addressing them once benefits the whole system.

---

## 7.2 The Shared Binding Constraints

Across the three chains, five constraints recur as decisive (drawing on the chain analyses and on cross-cutting evidence):

**1. Energy cost and reliability.** Metals processing (DRI, electro-refining, rolling) is electricity-intensive. Uganda's extra-large industrial tariff is around **5.5 US¢/kWh (5¢ off-peak) in 2025** (Electricity Regulatory Authority) — regionally competitive — but reliability and the cost of self-generation backup erode this advantage. The 2025 transition from UMEME to UEDCL as distributor is an opportunity to lock in industrial reliability commitments.

**2. Logistics and the landlocked penalty.** Logistics inefficiencies cost the economy an estimated **USD 827 million a year**, and logistics account for **18–20% of the sale price** of goods (sector estimates). For bulk, low-value-density products like steel and ore, this is a first-order competitiveness drag and a reason to favour domestic processing over raw-material export.

**3. Access to long-term finance.** Integrated metals and automotive plants are highly capital-intensive with long pay-back. Long-term finance is scarce and costly, and **2025 liquidity tightening — driven by government borrowing — has crowded out private credit** (KPMG, *Uganda Budget Brief 2025/26*). KMC's own FY25/26 financing gap illustrates the problem at the firm level.

**4. Skills.** Around **68% of manufacturers report difficulty finding workers with the technical skills they need** (Uganda Manufacturers Association). Metallurgy, process engineering, mechatronics, and EV skills are particularly scarce.

**5. Standards and quality.** Sub-standard imported steel and electrical cable distort the market and endanger users, while uneven UNBS enforcement penalises compliant domestic firms. Quality infrastructure is both a safety issue and a competitiveness lever.

---

## 7.3 Cross-Cutting Priority Actions

### A. Secure feedstock and stop raw-material leakage
Uganda exports raw iron ore (USD 35.8m in 2024) and leaves copper ore unmined, then imports the processed metal back. **Restrict or tax raw mineral-ore exports and prioritise domestic beneficiation**, coordinated with the upstream investments in Chapters 4–5. This single policy shift underpins the import-substitution case across both metals chains.

### B. Industrial parks, power, and shared infrastructure
Cluster metals and automotive activity in serviced industrial parks (Namanve, Jinja, and a mineral-processing zone near the resource base) with **reliable dedicated power, water, effluent treatment, and logistics**. Shared infrastructure reduces per-firm costs — directly addressing PCP Priority Area 3.

### C. An industrial-finance instrument
Establish or scale a **long-term industrial-finance facility** (through Uganda Development Bank and development partners) for capital-intensive metals and automotive investment, with blended/concessional terms to offset the crowding-out of private credit. Tie disbursement to the prioritized products in Chapters 4–6.

### D. Standards enforcement
Resource UNBS to **enforce standards on imported steel and electrical cable**, levelling the field for compliant domestic producers and protecting consumers. A quick win requiring administrative rather than capital resources.

### E. Skills compact
A targeted **skills programme in metallurgy, process engineering, mechatronics, and EV technology**, delivered through UIRI, Makerere, and TVET institutions in partnership with anchor firms (Tembo, Roofings, Cable Corporation, KMC).

### F. Demand-side anchors through public procurement
Use **government procurement** — infrastructure steel, electrical cable for grid expansion, and KMC buses for public fleets — as a guaranteed demand anchor that de-risks domestic investment.

---

## 7.4 Alignment to PCP Priority Areas

| PCP Priority Area | Report 1 cross-cutting actions |
|---|---|
| 1. Industrial policy implementation & institutional coordination | Feedstock/ore-export policy (A); standards enforcement (D); coordinated MTIC–MEMD–MoFPED action |
| 2. Mobilizing large-scale public & private investment | Industrial-finance instrument (C); investment portfolio (Ch.8); production-sharing model (Kilembe) |
| 3. Sustainable industrial parks & free zones | Parks, power, shared infrastructure (B) |
| 4. National standards, quality & accreditation | UNBS enforcement and capacity (D) |
| 5. Green industry, circular economy & Industry 4.0 | Slag/tailings reuse; EV manufacturing (KMC); scrap-based Electric Arc Furnace (EAF) steel |
| 6. Productive use of renewable energy & efficiency | Industrial power reliability (A/B); hydro-grid-aligned EV strategy |

---

## 7.5 Summary of Cross-Cutting Recommendations

1. **Retain and process minerals domestically** — tax/restrict raw ore exports; prioritise beneficiation (highest-leverage single action).
2. **Build serviced industrial parks with reliable power** for metals and automotive clusters.
3. **Create a long-term industrial-finance facility** to overcome the capital constraint.
4. **Enforce UNBS standards** on steel and cable imports (quick win).
5. **Launch a metals/automotive/EV skills compact.**
6. **Anchor demand through public procurement** (infrastructure steel, grid cable, KMC buses).

These six actions, taken together, address the shared constraints that hold back all three chains and convert Uganda's resource endowment and its new industrial anchors (Tembo, Devki, Kilembe redevelopment, KMC) into a competitive metals-to-mobility industrial system.
