workspace "MIDD — Manufacturing Industry Diagnostics Dashboard" "As-built 2026-06-23: Jerome uploads a document via the web uploader (no Git) -> the Claude CLI brain on Hetzner reads his intent and extracts the data into one PocketBase datastore -> the dashboard rebuilds from it. Ask MIDD is the scoped feedback/return channel. Domain midd-ug.com over HTTPS." {
    ...
    model {
        ...
        cloudflare = softwareSystem "Cloudflare (midd-ug.com)" "DNS + proxy + HTTPS (SSL Full) in front of all surfaces. Zero Trust Access GATES the 4 internal tool hosts (upload/ask/staging-upload/staging-ask) to a 3-person email allow-list; login method is One-time PIN (email code) added at Integrations->Identity providers, alongside the Cloudflare-account IdP (ADR-015). The 2 dashboards stay public; PocketBase keeps its own admin login." "External"
        ...
        midd = softwareSystem "MIDD Platform" "Document-to-dashboard pipeline with a scoped brain and feedback loop, staging + prod." {
            ...
            askmidd  = container "Ask MIDD (scoped brain)" "FastAPI chat. Team-gated /ask (Max-plan CLI, windowed memory) on staging :8220 / prod :8221; the gated tool can run a read-only analytics sandbox (ADR-020, analytics_sandbox.py) that executes model-written pandas/numpy/sklearn/matplotlib over the data as in-memory DataFrames, four safety layers (AST validation, restricted builtins, no PB/network handle, subprocess rlimit+timeout) so it queries anything but alters nothing. PUBLIC /api/ask (ADR-014): Haiku, rate-limited, browser memory, plain prose, served same-origin at midd-ug.com/api/ask; keeps the validated query_tool path (plan->validate->query->answer), no code execution; the tool supports count, list and GROUP (aggregate counts over a whitelisted dimension, server-side) so the public bot can answer breakdown/ranking questions. ADR-025 (revised): the pandas/numpy/sklearn sandbox is now released to the PUBLIC too, made safe by sanitized DataFrames (person/contact columns dropped), a hardened subprocess (unshare --net no-egress + setpriv drop-privilege, fail-closed), tight timeout/memory, and a concurrency cap; the authenticated tier runs the same sandbox over the full un-sanitized data. Both run in a no-repo sandbox with a sanitized PocketBase brief so they answer live data without leaking names. Deployed staging-first via deploy_brain.sh (ADR-019)." "Python/FastAPI + Claude CLI"
            ...
            orchestrator = container "Upload Orchestrator" "scripts/process_upload.py (importable; pure route()). On upload: CLI reads the intent, then routes by folder AND file type: manufacturing-overview + register PDF -> deterministic parse; manufacturing-overview + spreadsheet -> key_indicators agent (updates existing KPI-card slugs from a UBOS scorecard, additive-only); any value-chain folder -> LLM ingestion. Then rebuilds + publishes the dashboard, self-checks. Runs detached on the box, so it posts start/done/failed progress to the MTIC WhatsApp group via the bridge so operators are not left waiting blind (ADR-022; opt-in MIDD_NOTIFY, best-effort, never breaks the ingest)." "Python"
            keyIndicatorsAgent = container "Key Indicators Agent" "agents/key_indicators_agent.py. Loads the UBOS scorecard sheets as pandas DataFrames and has the Claude CLI write read-only code that COMPUTES the figure (e.g. manufactured trade = SITC sections 5,6,7,8 excluding 68 for the latest year), runs it in analytics_sandbox (ADR-020, no data-source handle; the sandbox exposes common exception classes so model try/except runs), and PATCHes the result to EXISTING key_indicators slugs. validate_updates coerces number-typed fields (pct) to floats and drops an unparseable one, so a display string like '45.1% of imports' can no longer 400 the whole write. Retries with error feedback; additive-only, never creates/renames/deletes a card (ADR-017). Safety core validate_updates unit-tested." "Python/Claude CLI + analytics_sandbox"
            ...
            pocketbase = container "PocketBase — Datastore" "The ONLY data source (ADR-017, enforced): generators read solely from here, no CSV/JSON fallback, and check_single_source.sh blocks any reintroduction in CI + pre-commit. industries = canonical establishment table (1 row per reg_number, updated in place; curated map factories merged as FAC-*). Plus value_chains, kpi_indicators, key_indicators, diagnostic_datapoints. Trade data is stored in HS codes only, with a HS->SITC translator table (ADR-021); SITC-defined indicators (manufactured / high-tech exports+imports) are COMPUTED on demand by translating HS->SITC and summing the official code sets, not stored twice. Prod :8090, staging :8091." "PocketBase/SQLite"
            ...
            ci = container "GitHub Actions CI" "On push: single-source guard (ADR-017) + pytest (ADR-018) gate the build -> build dashboard from PROD PocketBase (:8090) -> deploy-staging RE-RENDERS index.html from STAGING PocketBase (:8091) so the staging host reflects the data workshop (uploaded-but-unpromoted data) instead of prod -> promote rsyncs STAGING_DIR->PROD_DIR byte-for-byte then RE-RENDERS prod index.html from PROD PocketBase (:8090) so prod shows only promoted data (without this the staging-PB render leaks unreviewed data onto prod) -> auto-merge to main -> self-heal. Seed job is SCHEMA-ONLY (PB_SCHEMA_ONLY=1: ensures collections+fields, never re-seeds records, so a deploy can't clobber PocketBase; ADR-017). Drift Check RETIRED (ADR-023): the scheduled CSV-mirror drift-check + its cron trigger are removed; PB is authoritative (ADR-017) so the CSV mirror was perpetually behind and only produced 6-hourly review-PR noise. CI now runs on push/workflow_dispatch only. deploy-brain + deploy-uploader jobs deploy those box apps staging->verify->prod when their code changes (ADR-019). A concurrency group (per github.ref, cancel-in-progress:false) serialises runs so a burst of merges + the reseed self-dispatch chain do not run many redundant deploys at once and over-burn Actions minutes; note the '4-second, no-failed-step' phantom failures were GitHub refusing to start jobs on an Actions SPENDING-LIMIT hit, fixed in the org Billing settings, not in this workflow." "YAML"
        }
        ...
        dashboard -> pocketbase "Browser live-fetches chains, factories, treemaps + explorer trade/structure at page load; static bundle fallback if unreachable (ADR-024). DIRECTION (ADR-025): a thin service layer is introduced post-demo so the UI stops fetching PocketBase directly; PocketBase stays behind it this phase, Postgres can move behind it later without frontend change."
