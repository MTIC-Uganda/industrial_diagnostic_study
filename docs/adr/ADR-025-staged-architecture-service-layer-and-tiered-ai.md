...
4. AI analytics for everyone, made safe by tier (decision revised 2026-07-21):
   - Public assistant: a cheap model plus the validated, parameterised, server-side analytical tools (count, list, group, rank), AND the full pandas/numpy/sklearn/matplotlib sandbox (ADR-020) so the public can ask open-ended analytical questions, not only pre-shaped ones. Releasing arbitrary code execution to anonymous users is made safe by four public-tier additions on top of the existing sandbox guards: (a) SANITIZED DataFrames (person/contact columns dropped, so no code path can surface a person, which is Jerome's hard rule); (b) a hardened subprocess (unshare --net for no network egress, closing the pd.read_csv(url) SSRF/exfil hole, plus setpriv to drop to an unprivileged user), which FAILS CLOSED (never falls back to in-process); (c) tight per-request timeout and memory; (d) a small concurrency cap plus the existing per-IP and global rate limits so it cannot exhaust the shared box.
   - Authenticated power users: the same sandbox over the FULL (un-sanitized) data, for internal analysis that legitimately needs person/contact detail.
