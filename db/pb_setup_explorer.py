...
# Skip gracefully when the file doesn't exist yet (populated in a later step).
...
if TREND_CSV.exists():
    ...
    existing_trend = pb('GET', '/api/collections/explorer_trade_trend/records?perPage=500').get('items', [])
    ...
    print('\n── explorer_trade_trend: trade_trend.csv not found — skipping (run fetch_strategic_data.py first)')
...
if PARTNERS_CSV.exists():
    ...
    print('\n── explorer_trade_partners: trade_partners.csv not found — skipping (run fetch_strategic_data.py first)')
