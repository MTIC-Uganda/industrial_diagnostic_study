...
COLLECTIONS = [
    ...
    {
        # 6-year Uganda import trend (2019–2024) for each HS4 code, fetched from
        # UN Comtrade. Powers sparklines + CAGR badges in the Explorer detail panel.
        # One row per (hs4_code, year) pair.
        'name': 'explorer_trade_trend',
        'type': 'base',
        'listRule': '',
        'viewRule': '',
        'createRule': None,
        ...
        'schema': [
            text('hs4_code', required=True),
            num('year'),
            num('imports_uganda'),      # USD thousands
            num('unit_value_usd_t'),    # USD per tonne (null when weight unavailable)
            num('display_order'),
        ],
    },
    {
        # Top import source countries per HS4 code (2024). Fetched from UN Comtrade
        # partner-breakdown calls. Powers "Top import sources" bar charts in the
        # Explorer detail panel. One row per (hs4_code, rank) pair (rank 1–8).
        'name': 'explorer_trade_partners',
        'type': 'base',
        'listRule': '',
        'viewRule': '',
        'createRule': None,
        ...
        'schema': [
            ...
            text('partner_name'),
            num('partner_code'),
            num('imports_value_usd_k'),  # USD thousands
            num('display_order'),
        ],
    },
]
...
# ── 7. explorer_trade_trend ─────────────────────────────────────────────────
# Seeded from trade_trend.csv (generated by scripts/fetch_strategic_data.py).
# CSV-derived Comtrade reference data — nobody edits it directly in PocketBase,
# so it runs even in PB_SCHEMA_ONLY mode (same policy as explorer_trade_hs4).
# Skip gracefully when the file doesn't exist yet.

TREND_CSV = DATA / 'trade_trend.csv'
if TREND_CSV.exists():
    trade_trend_csv = load_csv('trade_trend.csv')
    print('\n── Seeding explorer_trade_trend ──')
    # Full replace so re-runs are idempotent even when rows change.
    existing_trend = pb('GET', '/api/collections/explorer_trade_trend/records?perPage=600').get('items', [])
    for r in existing_trend:
        pb('DELETE', f'/api/collections/explorer_trade_trend/records/{r["id"]}')
    if existing_trend:
        print(f'  Cleared {len(existing_trend)} existing rows')
    for i, r in enumerate(trade_trend_csv):
        payload = {
            'hs4_code': r['hs4_code'],
            'year': int(r['year']),
            'imports_uganda': float(r['imports_uganda']),
            'unit_value_usd_t': float(r['unit_value_usd_t']) if r.get('unit_value_usd_t') else None,
            'display_order': i,
        }
        pb('POST', '/api/collections/explorer_trade_trend/records', payload)
    print(f'  {len(trade_trend_csv)} trend rows seeded')
else:
    print('\n── explorer_trade_trend: trade_trend.csv not found — skipping')

# ── 8. explorer_trade_partners ───────────────────────────────────────────────
# Same policy: CSV-derived Comtrade reference data, runs even in schema-only mode.

PARTNERS_CSV = DATA / 'trade_partners.csv'
if PARTNERS_CSV.exists():
    trade_partners_csv = load_csv('trade_partners.csv')
    print('\n── Seeding explorer_trade_partners ──')
    existing_partners = pb('GET', '/api/collections/explorer_trade_partners/records?perPage=500').get('items', [])
    for r in existing_partners:
        pb('DELETE', f'/api/collections/explorer_trade_partners/records/{r["id"]}')
    if existing_partners:
        print(f'  Cleared {len(existing_partners)} existing rows')
    for i, r in enumerate(trade_partners_csv):
        payload = {
            'hs4_code': r['hs4_code'],
            'rank': int(r['rank']),
            'partner_name': r['partner_name'],
            'partner_code': int(r.get('partner_code', 0)),
            'imports_value_usd_k': float(r['imports_value_usd_k']),
            'display_order': i,
        }
        pb('POST', '/api/collections/explorer_trade_partners/records', payload)
    print(f'  {len(trade_partners_csv)} partner rows seeded')
else:
    print('\n── explorer_trade_partners: trade_partners.csv not found — skipping')
