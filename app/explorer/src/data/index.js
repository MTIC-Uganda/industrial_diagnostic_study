...
// Returns the matched HS4 code string for a given input text, or null.
// Used to resolve trend + partner data from the same keyword rules.
function matchInputHs4(text) {
  for (const chain of ALL_CHAINS) {
    const result = chain.matchInputHs4?.(text);
    if (result) return result;
  }
  return null;
}

// TRADE_TREND, TRADE_PARTNERS, and OPPORTUNITY_SCORES are global cross-chain
// datasets generated by scripts/generate_explorer_data.py from UN Comtrade data
// (ADR-017). They live in ironSteel.js (the primary generated chain file).
const TRADE_TREND        = ironSteel.TRADE_TREND        || {};
const TRADE_PARTNERS     = ironSteel.TRADE_PARTNERS     || {};
const OPPORTUNITY_SCORES = ironSteel.OPPORTUNITY_SCORES || {};
